...
const CONTRACT_TEMPLATES: Array[Dictionary] = [
	...
	{ "resource_id": "piece_avancee", "label": "Pieces avancees",  "qty_min": 1,  "qty_max": 3,  "bonus_factor": 1.5, "penalty": 400.0 },
]
...
func _ready() -> void:
	if TimeManager and not _day_connected:
...
func _generate_contract(day: int) -> void:
	# Un contrat sur deux est pour piece_base, un sur deux pour piece_avancee
	var template: Dictionary = CONTRACT_TEMPLATES[(day - 1) % CONTRACT_TEMPLATES.size()]
	var quantity: int = randi_range(int(template["qty_min"]), int(template["qty_max"]))
	...
	var contract: Dictionary = {
		...
		"day_issued": day,
		"day_deadline": day,           # à livrer dans la journée
